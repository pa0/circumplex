...
pdf.options(encoding = "CP1250.enc")                                 # polskie znaki na wykresach
...
library(readr)
codebook <- read_delim("dane surowe/codebook.csv", 
...
# korekty na danych
dane |> 
  mutate(across(all_of(c("change_selfesteem", "change_work", "change_relationships")), as.numeric)) -> dane
```
...
```{r rtt_alfa, child = "analiza-Rzetelnosc.Rmd"}
```
